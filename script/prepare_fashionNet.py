...
df["class_id"] = df.apply(
    lambda row: str(row["articleType"]) + "__" + str(row["id"]) + ".jpg", axis=1
)
    ...
    for f in tqdm(files, total=len(files)):
        if f.endswith(".jpg"):
            ...
            subset_folder = (
                ...
                else "images_background"
            )
            src = "{}/{}".format(root, f)
            dst = DATA_PATH + "/fashionNet/{}/{}/{}".format(
                subset_folder, class_name, image_name
            )
            shutil.copy(src, dst)
