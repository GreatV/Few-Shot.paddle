    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if not self.writer:
            ...
            self.writer = csv.DictWriter(self.csv_file, fieldnames=
                fieldnames, dialect=CustomDialect)
...
class EvaluateMetrics(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        logs = logs or {}
        logs.update(evaluate(self.model, self.dataloader, self.
            prepare_batch, self.metrics, self.loss_fn, self.prefix, self.
            suffix))
...
class ReduceLROnPlateau(Callback):
    ...
    def __init__(self, monitor='val_loss', factor=0.1, patience=10, verbose
        =0, mode='auto', min_delta=0.0001, cooldown=0, min_lr=0, **kwargs):
    ...
    def _reset(self):
        ...
        """
        if (self.mode == 'min' or self.mode == 'auto' and 'acc' not in self
            .monitor):
    ...
    def _reduce_lr(self, epoch):
        for i, param_group in enumerate(self.optimiser.param_groups):
            ...
            if old_lr - new_lr > self.min_delta:
                ...
                if self.verbose:
                    print(
                        'Epoch {:5d}: reducing learning rate of group {} to {:.4e}.'
                ...
                if current is None:
                    ...
                    if self.verbose > 0:
                        print(
                            ...
                            filepath))
                    self.best = current
                    paddle.save(obj=self.model.state_dict(), path=filepath)
                elif self.verbose > 0:
...
class LearningRateScheduler(Callback):
    ...
    def on_epoch_begin(self, epoch, logs=None):
        lrs = [self.schedule(epoch, param_group['lr']) for param_group in
            self.optimiser.param_groups]
        if not all(isinstance(lr, (float, np.float32, np.float64)) for lr in
            lrs):
            raise ValueError(
    ...
    def set_lr(self, epoch, lrs):
        for i, param_group in enumerate(self.optimiser.param_groups):
            ...
            if self.verbose:
                print(
                    'Epoch {:5d}: setting learning rate of group {} to {:.4e}.'
                    .format(epoch, i, new_lr))
