...
class NShotTaskSampler(paddle.io.Sampler):

    def __init__(self, dataset: paddle.io.Dataset, episodes_per_epoch: int=
        None, n: int=None, k: int=None, q: int=None, num_tasks: int=1,
        ...
        for _ in range(self.episodes_per_epoch):
            ...
            for task in range(self.num_tasks):
                if self.fixed_tasks is None:
                    episode_classes = np.random.choice(self.dataset.df[
                        'class_id'].unique(), size=self.k, replace=False)
                else:
                    episode_classes = self.fixed_tasks[self.i_task % len(
                        self.fixed_tasks)]
                    self.i_task += 1
                ...
                for k in episode_classes:
                    support = df[df['class_id'] == k].sample(shape=self.n)
                    support_k[k] = support
                    for i, s in support.iterrows():
                        batch.append(s['id'])
                query_k = {k: None for k in episode_classes}
                for k in episode_classes:
                    query = df[(df['class_id'] == k) & ~df['id'].isin(
                        support_k[k]['id'])].sample(shape=self.q)
                    query_k[k] = query
...
class EvaluateFewShot(Callback):
    ...
    def __init__(self, eval_fn: Callable, num_tasks: int, n_shot: int,
        k_way: int, q_queries: int, taskloader: paddle.io.DataLoader,
        prepare_batch: Callable, prefix: str='val_', **kwargs):
        ...
        self.prefix = prefix
        self.kwargs = kwargs
        self.metric_name = '{}{}-shot_{}-way_acc'.format(self.prefix, self.
            n_shot, self.k_way)
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for batch_index, batch in enumerate(self.taskloader):
            ...
            loss, y_pred = self.eval_fn(self.model, self.optimiser, self.
                loss_fn, x, y, n_shot=self.n_shot, k_way=self.k_way,
                q_queries=self.q_queries, train=False, **self.kwargs)
            seen += y_pred.shape[0]
...
def prepare_nshot_task(n: int, k: int, q: int) ->Callable:
    ...
    def prepare_nshot_task_(batch: Tuple[paddle.Tensor, paddle.Tensor]
...
def create_nshot_task_label(k: int, q: int) ->paddle.Tensor:
    ...
    """
    y = paddle.arange(start=0, end=k, step=1 / q).astype(dtype='int64')
    return y
